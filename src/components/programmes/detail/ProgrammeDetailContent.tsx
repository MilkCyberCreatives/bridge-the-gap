...
export default function ProgrammeDetailContent({
  ...
  const wrap = {
    hidden: {},
    show: {
      transition: reduceMotion ? { duration: 0 } : { staggerChildren: 0.08, delayChildren: 0.03 },
    },
  };
      ...
      <div className="container-tight">
        ...
        >
          <motion.div variants={item}>
            ...
            <p className="mt-3 text-base leading-relaxed text-black/70">{introText}</p>

            <div className="mt-6 space-y-4">
              {highlights.map((h) => (
                <div key={h.title} className="water-hover flex items-start gap-3 rounded-3xl border border-border bg-white/65 p-5 backdrop-blur-xl">
                  <CheckCircle className="mt-1 h-5 w-5 text-[rgb(var(--brand))]" />
                  <div>
                    <p className="font-extrabold text-black/85">{h.title}</p>
                    <p className="mt-1 text-sm leading-relaxed text-black/70">{h.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.div variants={item} className="md:pt-7">
            <div className="water-hover rounded-3xl border border-border bg-white/65 p-6 backdrop-blur-xl">
              <p className="text-sm font-extrabold tracking-tight text-black/85">What’s Included</p>
              <div className="mt-4 space-y-3">
                {includes.map((x) => (
                  <div key={x.title} className="flex items-start gap-3">
                    <span className="mt-1 h-2 w-2 rounded-full bg-[rgb(var(--brand))]" />
                    <div>
                      <p className="text-sm font-extrabold text-black/85">{x.title}</p>
            ...
            <div className="mt-4 rounded-3xl border border-border bg-white/45 p-6 text-sm text-black/70 backdrop-blur-xl">
              We align to CAPS & IEB outcomes and focus on steady improvement with clear feedback.
            </div>
          </motion.div>
        </motion.div>
        ...
        >
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold tracking-wide text-black/55">In Action</p>
        ...
        >
          <motion.div variants={item} className="mx-auto max-w-3xl text-center">
            <p className="text-xs font-semibold tracking-wide text-black/55">Programme FAQ</p>
